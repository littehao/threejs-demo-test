...
camera.position.set(35.04, 73.04, 216.46);//根据渲染范围尺寸数量级设置相机位置
...
document.getElementById('openBtn').addEventListener('click',function(){
    console.log('开门')
    const objLeft = model.getObjectByName('左边门')
    const objRight = model.getObjectByName('右边门')
    if(flag){
        flag = false
        objLeft.rotateY(Math.PI/2)
        objRight.rotateY(-Math.PI/2)
    } else {
        flag = true
        objLeft.rotateY(-Math.PI/2)
        objRight.rotateY(Math.PI/2)
